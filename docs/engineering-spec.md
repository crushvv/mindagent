...
- 单模态评分必须由大模型执行，不使用纯关键词规则作为主判断。
- 每个模态输出标签概率与置信度，供融合阶段使用。
...
- 关键词硬规则优先级高于融合：命中危险词（自杀/自残/伤人等）直接将 `emotionLabel` 置为 `risk`。
...
- 中文：`自杀`、`不想活`、`结束生命`、`伤害自己`、`杀人`
- 英文：`kill myself`、`suicide`、`hurt myself`
